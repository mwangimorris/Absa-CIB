...
class CurrentWeatherTests: XCTestCase {
    ...
    func testMandatoryDataValidity() {
        XCTAssertEqual(weatherResponse.main.temp, 284.2)
        XCTAssertEqual(weatherResponse.weather.first?.description, "few clouds")
        XCTAssertEqual(weatherResponse.name, "San Francisco")
    ...
    func testReceivedCoordinatesValidity() {
        XCTAssertEqual(weatherResponse.coord.lon, -122.4064)
        XCTAssertEqual(weatherResponse.coord.lat, 37.7858)
    }

    /// Ensure that the main temperature is converted to celsius correctly
    func testTemperatureConversionValidity() {
        XCTAssertEqual(weatherResponse.main.temp.toCelsius(), "11.05°C")
    }
